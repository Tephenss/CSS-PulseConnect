...
$sectionId = str_replace(' ', '-', trim((string) ($_GET['id'] ?? '')));
$sectionName = trim((string) ($_GET['name'] ?? 'Block'));
if ($sectionId === '') {
    header('Location: /teacher_sections');
    exit;
}
...
$sectionUrl = rtrim(SUPABASE_URL, '/') . '/rest/v1/sections'
    ...
    . '&id=eq.' . rawurlencode($sectionId)
    . '&limit=1';
$sectionRes = supabase_request('GET', $sectionUrl, $headers);
$sectionRows = $sectionRes['ok'] ? json_decode((string) ($sectionRes['body'] ?? ''), true) : null;
$sectionRow = is_array($sectionRows) && isset($sectionRows[0]) && is_array($sectionRows[0]) ? $sectionRows[0] : null;
if (!$sectionRow) {
    header('Location: /teacher_sections');
    exit;
}
if ($sectionName === '' || $sectionName === 'Block') {
    $sectionName = trim((string) ($sectionRow['name'] ?? 'Block'));
}

// Registered student accounts only — never password, never roster dump.
$url = rtrim(SUPABASE_URL, '/') . '/rest/v1/users'
    . '?select=id,first_name,middle_name,last_name,suffix,email,student_id,archived_at'
    . '&role=eq.student'
...
foreach ($students as $s) {
    ...
    $directory[] = [
        'student_no' => trim((string) ($s['student_id'] ?? '')),
...
render_header('Block: ' . $sectionName, $user);
?>

<div class="mb-6 flex items-center justify-between gap-3">
  <div>
    <h2 class="text-xl font-bold text-zinc-900 mb-1"><?= htmlspecialchars($sectionName) ?> Students</h2>
    <p class="text-sm text-zinc-500">Registered accounts: <?= count($directory) ?></p>
  </div>
  <a href="/teacher_sections" class="text-sm font-semibold text-zinc-600 hover:text-zinc-900 px-3 py-2 bg-zinc-100 hover:bg-zinc-200 rounded-lg transition-colors border border-zinc-200 inline-flex items-center gap-2">
      ...
      <table class="w-full text-left text-sm text-zinc-600 whitespace-nowrap">
        <thead class="bg-[#FAFAFA] text-zinc-800 font-semibold border-b border-zinc-200">
          <tr>
            ...
            <th class="px-6 py-4 rounded-tr-xl">Email</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-zinc-100">
          <?php if (count($directory) > 0): ?>
            <?php foreach ($directory as $row): ?>
              <?php
                ...
                $givenName = trim(implode(' ', array_filter([
                ...
                if ($lastName !== '' && $givenName !== '') {
                    ...
                    $displayName = $givenName !== '' ? $givenName : 'Student';
                }
                if ($suffix !== '') {
                    $displayName .= ' ' . $suffix;
                }
              ...
              <tr class="hover:bg-zinc-50 transition-colors group">
                ...
                <td class="px-6 py-4">
                  <div class="font-semibold text-zinc-900"><?= htmlspecialchars($displayName) ?></div>
                </td>
          ...
          <?php else: ?>
            <tr>
              <td colspan="3" class="px-6 py-12 text-center text-zinc-500 bg-white">
                <div class="flex flex-col items-center justify-center">
                  ...
                  <p class="font-medium text-zinc-800">No registered students</p>
                  <p class="text-xs mt-1 text-zinc-400">Students with created accounts in this block will appear here.</p>
                </div>
              </td>
            </tr>
          <?php endif; ?>
