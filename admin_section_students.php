    ...
    <div class="overflow-x-auto">
      <table class="w-full text-left text-sm text-zinc-600 whitespace-nowrap">
        ...
        <tbody class="divide-y divide-zinc-100">
          ...
          <?php else: ?>
            <tr>
              <td colspan="4" class="px-6 py-12 text-center text-zinc-500 bg-white">
...
document.getElementById('searchInput').addEventListener('input', function(e) {
    ...
    if (visibleCount === 0 && rows.length > 0) {
        if (!emptyRow) {
            ...
            tr.innerHTML = `<td colspan="4" class="px-6 py-8 text-center text-zinc-500 bg-white">No students match your search.</td>`;
...
document.querySelectorAll('.btnResetOne').forEach(btn => {
    btn.addEventListener('click', async () => {
        ...
        const confirmed = confirm(
            `Reset section for ${studentName}?\n\n` +
            'This will clear only this student\'s section assignment and require them to select again.'
        );
        if (!confirmed) return;
        ...
        try {
            ...
            if (!data.ok) throw new Error(data.error || 'Failed to reset student section');

            alert(`Section reset successful for ${studentName}.`);
            window.location.reload();
        } catch (err) {
            alert(err.message || 'Failed to reset student section.');
            btn.disabled = false;
