...
$renderEventCard = static function (array $e, bool $isFinished): void {
    ...
    if (strtolower((string) $for) === 'none') {
        $targetLabel = 'No Target';
    } elseif ((string) $for === '1') {
        $targetLabel = '1st Year';
    } elseif ((string) $for === '2') {
        $targetLabel = '2nd Year';
    } elseif ((string) $for === '3') {
        $targetLabel = '3rd Year';
    } elseif ((string) $for === '4') {
        $targetLabel = '4th Year';
    }
    ?>
...
<div class="mb-6 flex items-center justify-between">
  <div>
    ...
    <p class="text-zinc-600 text-sm">Browse published events and review finished ones without sending them straight to archive.</p>
  </div>
</div>
<div class="mb-5 flex flex-wrap items-center gap-2 rounded-2xl border border-zinc-200 bg-white p-2 shadow-sm w-fit">
  <button type="button" id="tabPublished" class="event-tab-btn rounded-xl bg-orange-600 px-4 py-2 text-sm font-bold text-white shadow-sm">
    Published
    <span class="ml-1.5 rounded-full bg-white/20 px-2 py-0.5 text-[11px]"><?= count($publishedEvents) ?></span>
  </button>
  <button type="button" id="tabFinished" class="event-tab-btn rounded-xl px-4 py-2 text-sm font-semibold text-zinc-600 hover:bg-zinc-100">
    Finished
    <span class="ml-1.5 rounded-full bg-zinc-100 px-2 py-0.5 text-[11px] text-zinc-600"><?= count($finishedEvents) ?></span>
  </button>
</div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
    <?php if (count($publishedEvents) === 0): ?>
      <div class="md:col-span-2 lg:col-span-3 rounded-2xl border border-dashed border-zinc-300 bg-zinc-50/80 p-12 text-center">
        ...
        <p class="text-sm text-zinc-600 max-w-md mx-auto">There are currently no upcoming or ongoing published events available in this list.</p>
      </div>
    <?php endif; ?>
...
<section id="finishedPanel" class="hidden">
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
    <?php if (count($finishedEvents) === 0): ?>
      <div class="md:col-span-2 lg:col-span-3 rounded-2xl border border-dashed border-zinc-300 bg-zinc-50/80 p-12 text-center">
        <div class="w-16 h-16 rounded-full bg-white border border-zinc-200 flex items-center justify-center mx-auto mb-4 shadow-sm">
          <svg class="w-8 h-8 text-zinc-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        </div>
        <h3 class="text-lg font-medium text-zinc-800 mb-1">No finished events yet</h3>
        <p class="text-sm text-zinc-600 max-w-md mx-auto">Completed events will stay here instead of being auto-archived right away.</p>
      </div>
    <?php endif; ?>
