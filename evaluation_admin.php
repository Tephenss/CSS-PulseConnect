    ...
    foreach ($questions as $question) {
        ...
        if ($fieldType === 'rating') {
            ...
            foreach ($distribution as $score => $scoreCount) {
                $pct = $count > 0 ? round(($scoreCount / $count) * 100) : 0;
                $bars[] = [
                    'score' => (string) $score,
                    'count' => $scoreCount,
                    'percent' => $pct,
    ...
    <div class="max-w-4xl mx-auto space-y-8">
      <?php foreach ($feedbackSections as $sectionIndex => $section): ?>
        <?php
          ...
          // Always show full year-level color legend (Google Forms style)
          $yearLegend = [];
          ...
          <?php else: ?>
              ...
              <div class="flex flex-col md:flex-row items-center gap-8 md:gap-12">
                <div class="w-56 h-56 shrink-0">
                  <?php if (count($piePaths) > 0): ?>
                    <svg viewBox="0 0 200 200" class="w-full h-full drop-shadow-sm" role="img" aria-label="Year level distribution pie chart">
                ...
                <div class="feedback-year-legend">
                  <?php foreach ($yearLegend as $leg): ?>
                    <div class="feedback-year-legend-item">
                      <span class="feedback-year-legend-dot" style="background: <?= htmlspecialchars((string) $leg['color']) ?>"></span>
                      <span class="text-sm font-medium text-zinc-800"><?= htmlspecialchars((string) $leg['label']) ?></span>
                    </div>
                  <?php endforeach; ?>
                </div>
              </div>
            ...
            <div class="rounded-2xl border border-zinc-200 bg-white p-5 space-y-5">
              ...
              <div>
                ...
                <div class="feedback-scroll-list space-y-2">
                  ...
                  <?php foreach (($section['rating_analytics'] ?? []) as $item): ?>
                    <?php
                      ...
                      if ($yMax <= 1) {
                          ...
                          $mag = (int) pow(10, max(0, (int) floor(log10($yMax))));
                          $yMax = (int) (ceil($yMax / $mag) * $mag);
                      }
                      ...
                      $yTicks = [];
                      for ($t = 0; $t <= 4; $t++) {
                          $yTicks[] = (int) round(($yMax / 4) * $t);
                      }
                      $yTicks = array_values(array_unique($yTicks));
                    ?>
                    <div class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
                      <div class="mb-4">
                        ...
                        <p class="text-sm text-zinc-500 mt-0.5"><?= $itemCount ?> responses · Avg <?= htmlspecialchars((string) ($item['avg'] ?? 0)) ?>/5</p>
                      </div>

                      <div class="flex gap-3">
                        <div class="w-8 shrink-0 relative" style="height: <?= $plotH ?>px; margin-top: <?= $labelHead ?>px;">
                          <?php foreach ($yTicks as $tick): ?>
                            <?php $topPct = $yMax > 0 ? (1 - ($tick / $yMax)) * 100 : 0; ?>
                            <span class="absolute right-0 text-xs font-medium text-zinc-400 -translate-y-1/2" style="top: <?= $topPct ?>%;"><?= $tick ?></span>
                          <?php endforeach; ?>
                        </div>

                        <div class="flex-1 min-w-0">
                          <div class="relative border-b border-zinc-300" style="height: <?= $plotH ?>px; margin-top: <?= $labelHead ?>px; overflow: visible;">
                            <?php foreach ($yTicks as $tick): ?>
                              <?php if ($tick === 0) {
                                  continue;
                              } ?>
                              <?php $topPct = $yMax > 0 ? (1 - ($tick / $yMax)) * 100 : 0; ?>
                              <div class="absolute left-0 right-0 border-t border-zinc-100 pointer-events-none" style="top: <?= $topPct ?>%;"></div>
                            <?php endforeach; ?>

                            <div class="absolute inset-0 flex items-end justify-between gap-2 px-1 sm:px-3">
                              <?php foreach (($item['bars'] ?? []) as $bar): ?>
                                <?php
                                  $barCount = (int) ($bar['count'] ?? 0);
                                  $barPct = (int) ($bar['percent'] ?? 0);
                                  $barH = $yMax > 0
                                      ? (int) round(($barCount / $yMax) * $plotH)
                                      : 0;
                                  if ($barCount > 0 && $barH < 8) {
                                      $barH = 8;
                                  }
                                ?>
                                <div class="relative flex-1 h-full min-w-0 mx-auto" style="max-width: 64px; overflow: visible;">
                                  <div
                                    class="absolute text-xs font-bold text-zinc-700 whitespace-nowrap z-10"
                                    style="left: 50%; transform: translateX(-50%); bottom: <?= $barH + 6 ?>px;"
                                  >
                                    <?= $barCount ?> (<?= $barPct ?>%)
                                  </div>
                                  <?php if ($barH > 0): ?>
                                    <div
                                      class="feedback-bar-fill absolute"
                                      style="height: <?= $barH ?>px; bottom: 0; left: 50%; transform: translateX(-50%); background: #7f1d1d;"
                                      title="<?= htmlspecialchars((string) ($bar['score'] ?? '')) ?>: <?= $barCount ?> (<?= $barPct ?>%)"
                                    ></div>
                                  <?php endif; ?>
                                </div>
                              <?php endforeach; ?>
                          ...
                          <div class="flex items-start justify-between gap-2 px-1 sm:px-3 mt-2">
                            <?php foreach (($item['bars'] ?? []) as $bar): ?>
                              <div class="flex-1 text-center text-xs font-semibold text-zinc-600 mx-auto" style="max-width: 64px;"><?= htmlspecialchars((string) ($bar['score'] ?? '')) ?></div>
                            <?php endforeach; ?>
                          </div>
                          <div class="flex items-center justify-between text-xs font-medium text-zinc-500 px-1 sm:px-3 mt-1">
                            <span>Poor</span>
                ...
                <div class="bg-zinc-50 p-4 space-y-4">
                  <?php foreach (($section['comments'] ?? []) as $item): ?>
                    <div class="rounded-2xl border border-zinc-200 bg-white p-5">
                      ...
                      <div class="feedback-scroll-list space-y-2" style="max-height: 16rem;">
                        <?php foreach (($item['responses'] ?? []) as $response): ?>
                          <div class="rounded-xl bg-zinc-100 px-3 py-2.5">
                            <div class="text-xs font-bold uppercase tracking-wide text-zinc-500 mb-1">
                              <?= htmlspecialchars((string) ($response['student_no'] ?? '—')) ?> · <?= htmlspecialchars((string) ($response['student_name'] ?? 'Student')) ?>
                            </div>
