...
<style>
  ...
  @keyframes logo-glow {
    ...
    0%,
    100% {
      text-shadow: 0 0 10px rgba(251, 146, 60, 0.8), 0 0 20px rgba(249, 115, 22, 0.6);
      color: #fff;
    }

    50% {
      text-shadow: 0 0 15px rgba(216, 180, 254, 1), 0 0 30px rgba(251, 146, 60, 0.8);
      color: #ffedd5;
    }
  }
  ...
  @keyframes mac-screen-content {
    ...
    0%,
    13.32% {
      background-image: url('assets/BSIT.png');
    }
  ...
  @keyframes mac-rotate {
    ...
    25% {
      transform: rotateX(-50deg) rotateY(150deg) rotateZ(0deg);
    }
...
<!-- Welcome Banner -->
<div
  class="relative overflow-hidden rounded-3xl border border-zinc-200 bg-gradient-to-br from-white via-orange-50/40 to-red-50/30 p-6 md:p-8 mb-8 shadow-sm">
  <div
    class="absolute -top-24 -right-24 w-64 h-64 bg-orange-400/10 blur-3xl rounded-full pointer-events-none overflow-hidden">
  </div>
  <div
    class="absolute -bottom-24 -left-24 w-64 h-64 bg-red-400/10 blur-3xl rounded-full pointer-events-none overflow-hidden">
  </div>
  ...
  <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 min-h-[140px]">
    <div class="flex-1">
      ...
      <p class="text-zinc-600 text-sm md:text-base max-w-xl leading-relaxed">
        <?php if ($role === 'student'): ?>
          Ready to explore? Register for upcoming events, access your e-tickets securely, and track your attendance
          progress across the semester.
        <?php elseif ($role === 'teacher'): ?>
          Manage your classes effectively. Create engaging events, monitor registrations, and seamlessly scan participant
          QR tickets for accurate attendance.
        <?php else: ?>
          System dashboard active. Maintain complete oversight of all events, manage platform users, analyze engagement
          metrics, and issue certificates.
        <?php endif; ?>
      </p>
    </div>
          ...
          <div class="macbody">
            <div class="face-one">
              ...
              <div class="keyboard mac-keyboard">
                <?php for ($k = 0; $k < 59; $k++): ?>
                  <div class="key <?= $k == 5 ? 'space' : '' ?>"></div><?php endfor; ?>
                <?php for ($k = 0; $k < 16; $k++): ?>
                  <div class="key f"></div><?php endfor; ?>
              </div>
            </div>
  ...
  <div class="lg:col-span-2 space-y-4">
    <div class="flex items-center justify-between mb-1">
      <div class="flex items-center gap-2">
        <div class="w-8 h-8 rounded-lg bg-orange-100 border border-orange-200 flex items-center justify-center">
          <svg class="w-4 h-4 text-orange-700" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round"
              d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" />
          </svg>
        </div>
      ...
      <a href="/events.php"
        class="group flex items-center gap-1.5 text-sm font-medium text-orange-700 hover:text-orange-900 transition-colors">
        View all
    ...
    <?php if (count($upcomingList) === 0): ?>
      <div class="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50/80 p-10 text-center">
        <div
          class="w-14 h-14 rounded-full bg-white border border-zinc-200 flex items-center justify-center mx-auto mb-3 shadow-sm">
    ...
    <?php foreach ($upcomingList as $e):
      ...
      <a href="/event_view.php?id=<?= htmlspecialchars((string) ($e['id'] ?? '')) ?>"
        ...
        <!-- Icon -->
        <div
          class="w-12 h-12 rounded-xl <?= $iconBg['bg'] ?> border <?= $iconBg['border'] ?> flex items-center justify-center flex-shrink-0">
          <svg class="w-6 h-6 <?= $iconBg['icon'] ?>" fill="none" stroke="currentColor" stroke-width="1.8"
            viewBox="0 0 24 24">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-start justify-between gap-2 mb-0.5">
            <h4 class="text-sm font-bold text-zinc-900 group-hover:text-orange-800 transition-colors truncate">
              <?= htmlspecialchars((string) ($e['title'] ?? 'Event')) ?></h4>
            <span
              class="text-[10px] font-bold uppercase tracking-wider rounded-full border px-2 py-0.5 flex-shrink-0 <?= $pillColor ?>"><?= htmlspecialchars($eTypePill) ?></span>
          </div>
          <p class="text-xs text-zinc-500 mb-1">
            <?php if (!empty($e['target_audience'])): ?><span class="font-medium text-zinc-600">For:
                <?= htmlspecialchars((string) $e['target_audience']) ?></span> · <?php endif; ?>
        ...
        <svg
          class="w-4 h-4 text-zinc-400 group-hover:text-orange-500 group-hover:translate-x-0.5 transition-all flex-shrink-0"
      ...
      <div class="grid grid-cols-7 gap-y-1 relative" id="calGrid">
        ...
        <?php for ($day = 1; $day <= $daysInMonth; $day++):
          ...
          <div class="relative flex flex-col items-center group/day">
            <button class="w-7 h-7 rounded-full text-xs font-semibold flex items-center justify-center transition-all
                <?= $isToday ? 'bg-orange-600 text-white shadow-md shadow-orange-200 font-bold' : 'hover:bg-zinc-100 text-zinc-700' ?>
                <?= $hasEvent && !$isToday ? 'font-bold text-orange-700' : '' ?>" type="button" <?= $hasEvent ? 'data-has-event="1"' : '' ?>>
              <?= $day ?>
            </button>
            <!-- Event Dot -->
            <?php if ($hasEvent):
              ...
              if (in_array('bg-emerald-500', $dots))
                $finalDot = 'bg-emerald-500';
              elseif (in_array('bg-orange-500', $dots))
                $finalDot = 'bg-orange-500';
              else
                $finalDot = 'bg-zinc-400';
              ?>
            ...
            <?php if ($hasEvent): ?>
              <div
                class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 hidden group-hover/day:block pointer-events-none"
                style="min-width:160px; max-width:220px;">
                <div class="bg-zinc-900 text-white rounded-xl shadow-xl p-3 text-left">
                  <p class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5">
                    <?= date('M d', mktime(0, 0, 0, $calMonth, $day, $calYear)) ?></p>
