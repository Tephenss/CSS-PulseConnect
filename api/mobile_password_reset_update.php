...
if (!api_rate_limit_allow('mobile_pw_reset_update:' . $clientIp, 10, 300)) {
...
if (!$user || empty($user['id'])) {
    json_response(['ok' => false, 'error' => 'Invalid reset session.'], 400);
}
...
try {
    $tokenExpires = new DateTimeImmutable($tokenExpiresRaw);
} catch (Throwable $e) {
...
// Revoke all mobile sessions for this user after password change.
supabase_request(
    'PATCH',
    rtrim(SUPABASE_URL, '/') . '/rest/v1/mobile_sessions?user_id=eq.' . rawurlencode($userId) . '&revoked_at=is.null',
    $updateHeaders,
    json_encode(['revoked_at' => $now->format(DATE_ATOM)], JSON_UNESCAPED_SLASHES)
);
